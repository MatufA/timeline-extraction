...
def generate_responses(model: genai.GenerativeModel, platinum_text_prepared_path: Path, prompt_path: Path,
                       results_path: Path):
    ...
    with results_path.open('a') as file:
        for record in tqdm(records, desc='Platinum Text evaluation', position=0):
            for trail in trange(5, desc=f'Processing record: {record["doc_id"]}', position=1, leave=False):
                ...
                record['prompt_token_count'] = response.usage_metadata.prompt_token_count
                record['candidates_token_count'] = response.usage_metadata.candidates_token_count
                record['trail'] = trail
