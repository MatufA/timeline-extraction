...
from full_temporal_relation.prompts.Prompt import PairwisePrompt
...
class TemporalRelationsToDrop(BaseModel):
    ...
    unique_ids: List[int] = Field(..., description='a list of unique ids of temporal relation')

class OpenAIClient(LLModel):
    def __init__(self, model_name: str = "gpt-4o-mini", use_formate: bool = False, use_dot_graph: bool = False , 
                 response_format: Optional[BaseModel] = None, *args, **kwargs):
...
def generate_batch_inference_file(mode: str, records_base_path: Path, output_dir_path: Path, 
                                  use_few_shot: bool, use_vague: bool, is_full_text: bool, 
                                  model_name: str = "gpt-4o-mini", n_trails: int = 5):
    ...
    output_batch_file = output_dir_path / f'{mode}_{vague_ref}_gpt_batch_{model_name}_te3-platinum_{context_ref}_format.jsonl'
    ...
    with output_batch_file.open('w') as obf:
        ...
        for record in tqdm(records, desc='Text evaluation', position=0, leave=True):
            ...
            for trail in range(n_trails):
                unique_id = '-'.join([str(trail), record['doc_id']] + record['relations'][0])
...
def push_batch_job(batch_file: Path):
    ...
    batch_file_out = batch_file.parent.parent.parent/ 'output' / method / batch_file_name
    ...
    for method in ['zero-shot', 'few-shot']:
        for use_vague in [True, False]:
            for is_full_text in [True, False]:
                ...
                context_ref = 'full_context' if is_full_text else 'minimal_context'
                vague_ref = 'w_vague' if use_vague else 'wo_vague'
                push_batch_job(output_method_path / f'{mode}_{vague_ref}_gpt_batch_{model_name}_te3-platinum_{context_ref}_format.jsonl')
