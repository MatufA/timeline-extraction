...
def count_tokens(text_path: Path, prompt_params: List[str], model_name: str, is_few_shot: bool=False):
    ...
    for record in records:
        messages = prompt_template.generate_dict_prompt(**{p: record[p] for p in prompt_params})
    ...
    for mode in modes:
        for is_few_shot in [True, False]:
            for model_name in [
                ...
                n_tokens = count_tokens(text_path, prompt_params, model_name, is_few_shot)
                print(f'mode={mode}, model name={model_name}, n_tokens={n_tokens}, is_few_shot={is_few_shot}')